...
#define TFT_LVGL_LAYER_DISP_RES  \
    (TFT_LVGL_LAYER_DISP_HOR_RES) * (TFT_LVGL_LAYER_DISP_VER_RES) * 2
...
// LVGL recommend to choose 1/10 screen size as drawing buffer size
// Using two buffer which one for DMA transfer data and another for MCU fill data
// static lv_color_t tft_disp_buf1[TFT_LVGL_LAYER_DISP_RES/10];
// static lv_color_t tft_disp_buf2[TFT_LVGL_LAYER_DISP_RES/10];
static lv_color_t tft_disp_buf1[240*10];
...
void tft_lvgl_layer_init(void)
{
    ...
    // lv_disp_draw_buf_init(&tft_disp_buf, tft_disp_buf1, tft_disp_buf2, TFT_LVGL_LAYER_DISP_RES/10);
    lv_disp_draw_buf_init(&tft_disp_buf, tft_disp_buf1, NULL, 240*10);
    ...
    lv_disp_t *tft_disp = lv_disp_drv_register(&tft_disp_drv);
...
static void tft_flush_cb(struct _lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    uint32_t bufsize = disp_drv->draw_buf->size;
